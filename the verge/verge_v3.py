...
ids = ['longform', 'reviews', 'tech', 'science', 'entertainment',\
       ...
       'design/archives', 'design', 'us-world', 'us-world/archives']
            ...
            for i in a:
                try:
                    if 'theverge.com' in i['href'] and i['href'].count('/') >4:
                        ...
                        if local_queue[-2] == local_queue[-1]:
                            break
                except:
                    pass
            if count_links ==0 :
...
class TZ(tzinfo):
    def utcoffset(self, dt): return timedelta(minutes=0)
...
def thevergeArticles(i, queue, debug, minDelay, maxDelay, b):
    while True:
        ...
        if debug:
            sys.stdout.write('Visting verge url :: ' + url + '\n' + '\n')

        logging.info('Visting verge url :: ' + url + '\n' + '\n')
        ...
        try:
            ...
            publish_date = ''
        try:
            date = str(datetime(publish_date.year, publish_date.month, publish_date.day, tzinfo=TZ()).isoformat()).replace(':00+00:00', '+00:00')
        except Exception,e:
            date = ''
...
if __name__ == "__main__":
    ...
    try:
        for worker in dataThreads:
            worker.join(10)
        print 'Closing text grabber thread. Work done'
    ...
    try:
        for worker in threads:
            worker.join(10)
        print 'Closing link grabber thread. Work done'
