...
def FFMPEGCommand(file_, videoFrameRate):
    ...
    """
    command = 'ffmpeg -y -i "'+file_+'" -map 0:1 -c:a copy -t 5 -map_metadata -1 audio0.mp4 -map 0:1 -c:a ac3 -t 5 -map_metadata -1 audio1.mp4 -map 0:1 -c:a aac -ac 2 -ab 320k -t 5 -map_metadata -1 audio2.mp4 -map 0:0 -c:v libx265 -x265-params "profile=main:level=4.1:keyint=' + str(4*int(videoFrameRate[:-1])) + ':min-keyint=' + str(4*int(videoFrameRate[:-1])) + ':scenecut=0" -crf 18 -sc_threshold 0 -flags +cgop -movflags faststart -pix_fmt yuv420p -t 5 -map_metadata -1 video1080hevc.mp4 -map 0:0 -c:v libx264 -x264opts keyint=' + str(4*int(videoFrameRate[:-1])) + ':min-keyint=' + str(4*int(videoFrameRate[:-1])) + ':no-scenecut -crf 18 -sc_threshold 0 -flags +cgop -movflags faststart -pix_fmt yuv420p -t 5 -map_metadata -1 video1080.mp4 -map 0:0 -c:v libx264 -x264opts keyint=' + str(4*int(videoFrameRate[:-1])) + ':min-keyint=' + str(4*int(videoFrameRate[:-1])) + ':no-scenecut -sc_threshold 0 -flags +cgop -profile:v baseline -level 4.1 -crf 18 -maxrate 4000k -bufsize 4000k -vf "scale=min(1280\,iw):trunc(ow/a/2)*2" -movflags faststart -pix_fmt yuv420p -t 5 -map_metadata -1 video720.mp4 -map 0:0 -c:v libx264 -x264opts keyint=' + str(4*int(videoFrameRate[:-1])) + ':min-keyint=' + str(4*int(videoFrameRate[:-1])) + ':no-scenecut -sc_threshold 0 -flags +cgop -profile:v baseline -level 4.1 -crf 18 -maxrate 1500k -bufsize 1500k -vf "scale=min(854\,iw):trunc(ow/a/2)*2" -movflags faststart -pix_fmt yuv420p -t 5 -map_metadata -1 video480.mp4'
...
def MP4BoxCommand(series_title, season_title, episode_title):
    ...
    command =  "MP4Box -dash 4000 -rap -frag-rap -profile onDemand -out " + \
              getHash(series_title +'-' + season_title + '-' + episode_title)+ "/dash.mpd video1080hevc.mp4 video1080.mp4 video720.mp4 video480.mp4 audio0.mp4 audio1.mp4 audio2.mp4"
    status = os.system(command)
    return status
...
def updateJsonFile(series_title, series_summary, series_release_date, series_art, series_thumb, \
    ...
    series_exists = 0
    data['groups'][0]['groupId'] = "14501699205860001"
    data['groups'][0]['name'] = "Recently Added"
    data['groups'][0]['cloudStorageId'] = 1
    series_all = data['groups'][0]['items']
    for series in series_all:
        if series['id'] == getHash(series_title):
            series_exists += 1
            ix_series = series_all.index(series)
            print 'ix_series', ix_series
            # series exists
            ...
            for season_ in seasons_all:
                if getHash(series_title + '-' + season_title) == season_['id']:
                    ...
                    print 'ix_season', ix_season
                    ## season exists
                    ## check if episode exists
                    episodes_all = data['groups'][0]['items'][ix_series]['seasons'][ix_season-1]['episodes']
                    episode_exists = 0
...
if __name__ == "__main__":    
    ...
    #downloadImage(series_title, '', '', imageBaseName, series_thumb), downloadImage(series_title, '', '', posterFileName, series_art)
    ...
    for item in returnData:
        ...
        #downloadImage(series_title, season_title, episode_title, imageBaseName, episode_thumb)
    
        #status, stat_dict = FFMPEGCommand(episode_file_, episode_videoFrameRate)
        status = 0
        if status == 0:
            #status1, stat_dict = MP4BoxCommand(season_title, season_title, episode_title)
            status1 = 0
            if status1 == 0:
                pass
