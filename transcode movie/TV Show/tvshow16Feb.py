...
def encrypt_file(message, key, iv):
    message = pad(message)
    cipher = AES.new(key, AES.MODE_CBC, iv)
    return iv + cipher.encrypt(message)

def encrypt(file_name, cipher_mode, key, iv):
    with open(file_name, 'rb') as fo:
        plaintext = fo.read()
    enc = encrypt_file(plaintext, key, iv)
...
def downloadImage(series_title, season_title, episode_title, name, url, cipher_mode, key, iv, USE_ENCRYPTION):
    ...
    if series_title and not season_title and not episode_title:
        ...
        del response
        if USE_ENCRYPTION: encrypt(getHash(series_title)+ "/"+ name +'_.jpg', getHash(series_title)+ "/"+ name +'.jpg', cipher_mode, key, iv)
    ...
    if series_title and season_title and not episode_title:
        ...
        with open(getHash(series_title) + '/' + getHash(season_title) + "/"+ name +'_.jpg', 'wb') as out_file:
            shutil.copyfileobj(response.raw, out_file)
        del response
        if USE_ENCRYPTION: encrypt(getHash(series_title) + '/' + getHash(season_title) + "/"+ name +'_.jpg', getHash(series_title) + '/' + getHash(season_title) + "/"+ name +'.jpg', cipher_mode, key, iv)
    ...
    if series_title and season_title and episode_title:
        ...
        with open(getHash(series_title) + '/' + getHash(season_title) + '/' + getHash(episode_title)+ "/"+ name +'_.jpg', 'wb') as out_file:
            shutil.copyfileobj(response.raw, out_file)
        del response
        if USE_ENCRYPTION: encrypt(getHash(series_title) + '/' + getHash(season_title) + '/' + getHash(episode_title)+ "/"+ name +'_.jpg', getHash(series_title) + '/' + getHash(season_title) + '/' + getHash(episode_title)+ "/"+ name +'.jpg', cipher_mode, key, iv)
